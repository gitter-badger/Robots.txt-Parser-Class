...
class RobotsTxtParser
{
    ...
    /**
     * Check an array of URLs
     *
     * @param array $urlArray
     * @param string|null $userAgent
     * @return array
     */
    public function checkURLs($urlArray, $userAgent = null)
    {
        $this->urls = array_merge($urlArray, $this->urls);
        $this->setUserAgent($userAgent);
        $duplicates = $this->findDuplicates();
        $array = [];
        foreach ($urlArray as $this->url) {
            $array[$this->url][self::DIRECTIVE_ALLOW] = $this->isAllowed($this->url, $this->userAgent_match);
            $array[$this->url][self::DIRECTIVE_DISALLOW] = $this->isDisallowed($this->url, $this->userAgent_match);
            $array[$this->url][self::DIRECTIVE_CRAWL_DELAY] = $this->getDelay($this->userAgent_match, self::DIRECTIVE_CRAWL_DELAY);
            $array[$this->url][self::DIRECTIVE_CACHE_DELAY] = $this->getDelay($this->userAgent_match, self::DIRECTIVE_CACHE_DELAY);
            $array[$this->url]['duplicate'] = in_array($this->url, $duplicates);
            // One signal to rule them all
            if ($array[$this->url][self::DIRECTIVE_DISALLOW]) {
                // Disallow
                $array[$this->url]['signal'] = self::DIRECTIVE_DISALLOW;
            } elseif ($array[$this->url]['duplicate']) {
                // Duplicate
                $array[$this->url]['signal'] = 'duplicate';
            } else {
                // Allow
                $array[$this->url]['signal'] = self::DIRECTIVE_ALLOW;
            }
        }
        return $array;
    }
    ...
    /**
     * Find URLs considered as duplicates
     *
     * @return array
     */
     ...
     * @return array
     */
    public function getDetails($userAgent = null)
    {
        $array = [];
        if (isset($userAgent)) {
            $this->setUserAgent($userAgent);
            $array[self::DIRECTIVE_USERAGENT] = $this->userAgent_match;
            $array[self::DIRECTIVE_CRAWL_DELAY] = $this->getDelay($this->userAgent_match, self::DIRECTIVE_CRAWL_DELAY);
